<template>
    <div v-if="isUserLoggedIn" class="main" :style="mainStyle">
      <UsuarioComp v-if="tipoUsuario === 'Admin'" />
      <DocenteComp v-if="tipoUsuario === 'Docente'" />
      <AlumnoComp v-if="tipoUsuario === 'Alumno'" />
      <router-view></router-view>
    </div>
    <div v-else class="main" :style="mainStyle">
      <h1>Portal Académico<br/>La Salle<br/>Universidad</h1>
      <form @submit="onSubmit" class="cart flex flex-column align-items-center gap-3 p-fluid">
        ...
        <div class="field">
          <input type="password" class="form-control" placeholder="Introduce tu contraseña" id="password" v-model="passwordInput" />
        </div>
        <small class="text-danger" id="text-error">{{ errorMessage || '&nbsp;' }}</small>
        <button type="submit" class="btn btn-primary">Iniciar sesión</button>
      </form>
      <a href="#">¿Olvidaste tu contraseña?</a><br/>
      <img src="../assets/google.svg" alt="Iniciar sesión con Google" />
      <img src="../assets/facebook.svg" alt="Iniciar sesión con Facebook" />
      <router-view></router-view>
    </div>
